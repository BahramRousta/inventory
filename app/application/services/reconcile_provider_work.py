...
class ReconcileProviderWorkService:
    def __init__(
        self, *, uow_factory: Callable[[], UnitOfWork], provider_gateways: ProviderGatewayRegistry
    ) -> None:
    ...
    async def reconcile_hold(self, work: ClaimedExternalHoldRecord) -> bool:
        result = await self._lookup(
            work.provider_id, f"{work.reservation_id}:{work.stock_source_id}:RESERVE"
        )
        async with self._uow_factory() as uow:
            if result.outcome == ProviderReservationLookupOutcome.UNKNOWN:
                persisted = await uow.reservations.return_hold_claim_to_unknown(
                    work.reservation_id, work.stock_source_id, work.claim_token
                )
            else:
                status = (
                    ReservationLineStatus.HELD
                    if result.outcome == ProviderReservationLookupOutcome.RESERVED
    ...
    async def reconcile_release(self, work: ClaimedExternalReleaseRecord) -> bool:
        ...
        async with self._uow_factory() as uow:
            if result.outcome == ProviderReservationLookupOutcome.UNKNOWN:
                persisted = await uow.reservations.return_release_claim_to_unknown(
                    work.reservation_id, work.stock_source_id, work.claim_token
                )
            elif result.outcome == ProviderReservationLookupOutcome.NOT_RESERVED:
                ...
                persisted = await uow.reservations.return_release_claim_to_pending(
                    work.reservation_id, work.stock_source_id, work.claim_token
                )
            if persisted:
                await uow.commit()
    ...
    async def _lookup(self, provider_id, reservation_key: str) -> ProviderReservationLookupResult:
        provider = self._providers.get(provider_id)
        if provider is None:
            return ProviderReservationLookupResult(ProviderReservationLookupOutcome.UNKNOWN)
        try:
            return await provider.get_reservation(reservation_key=reservation_key)
        except Exception:
            return ProviderReservationLookupResult(ProviderReservationLookupOutcome.UNKNOWN)
